...
describe('View tests', () => {
  ...
  test('displaySlider should create secondHandle and secondValue if range is true', () => {
    ...
    expect(
      view
        .getViewElements()
        .secondHandle.getSliderHandle()
        .classList.contains('ng-slider__handle')
    ).toBeTruthy();
    expect(
      view
        .getViewElements()
        .secondValue.getCurrentValue()
        .classList.contains('ng-slider__current-value')
    ).toBeTruthy();
    expect(
      view
        .getSlider()
        ?.contains(view.getViewElements().secondHandle.getSliderHandle())
    ).toBeTruthy();
    expect(
      view
        .getViewElements()
        .secondHandle.getSliderHandle()
        .contains(view.getViewElements().secondValue.getCurrentValue())
    ).toBeTruthy();
  });
  ...
  test('displaySlider should hide first and second values if it says so in options', () => {
    ...
    view.displaySlider(options);
    expect(
      view.getViewElements().firstValue.getCurrentValue().style.display
    ).toBe('none');
    expect(
      view.getViewElements().secondValue.getCurrentValue().style.display
    ).toBe('none');
  });
  ...
  test('destroySlider should call destroy labels', () => {
    const destroyMock = jest.fn(view.getViewElements().labels.destroy);
  ...
  test('setHandles should call setHandle for each handle', () => {
    const setHandleMock = jest.fn(view.getViewElements().firstHandle.setHandle);
  ...
  test('changeValue should call setCurrentValue in firstValue and secondValue', () => {
    const setCurrentFirstValueMock = jest.fn(
      view.getViewElements().firstValue.setTextOfCurrentValue
    );
    const setCurrentSecondValueMock = jest.fn(
      view.getViewElements().secondValue.setTextOfCurrentValue
    );
    view.getViewElements().secondValue.setTextOfCurrentValue =
      setCurrentSecondValueMock;
    view.getViewElements().firstValue.setTextOfCurrentValue =
      setCurrentFirstValueMock;
  ...
  test('changeValue should call moveHandle in firstHandle and secondHandle', () => {
    const moveFirstHandleMock = jest.fn(
      view.getViewElements().firstHandle.moveHandle
    );
    const moveSecondHandleMock = jest.fn(
      view.getViewElements().secondHandle.moveHandle
    );
  ...
  test('makeVertical should add css classes with vertical postfix of elements of slider', () => {
    view.makeVertical();
    expect(
      view.getSlider()?.classList.contains('ng-slider_vertical')
    ).toBeTruthy();
    expect(
      view
        .getViewElements()
        .firstHandle.getSliderHandle()
        .classList.contains('ng-slider__handle_vertical')
    ).toBeTruthy();
    expect(
      view
        .getViewElements()
    ...
    expect(
      view
        .getViewElements()
        .sliderTrack.getSliderTrack()
        .classList.contains('ng-slider__track_vertical')
    ).toBeTruthy();
    expect(
      view
        .getViewElements()
        .labels.getLabels()
        .classList.contains('ng-slider__values_vertical')
    ).toBeTruthy();
    expect(
      view
        .getViewElements()
        .firstValue.getCurrentValue()
        .classList.contains('ng-slider__current-value_vertical')
    ).toBeTruthy();
    expect(
      view
        .getViewElements()
  ...
  test('makeVertical should add default styles for some properties', () => {
    view.makeVertical();
    expect(
      view.getViewElements().sliderTrack.getSliderTrack().style.width
    ).toBe('5px');
    expect(view.getViewElements().sliderTrack.getSliderTrack().style.left).toBe(
      '0px'
    );
    expect(
      view.getViewElements().firstHandle.getSliderHandle().style.left
    ).toBe('-5px');
    expect(
      view.getViewElements().secondHandle.getSliderHandle().style.left
    ).toBe('-5px');
  });
  ...
  test('makeHorizontal should remove css classes with vertical postfix of elements of slider', () => {
    ...
    expect(
      view
        .getViewElements()
        .sliderTrack.getSliderTrack()
        .classList.contains('ng-slider__slider-track_vertical')
    ).toBeFalsy();
    expect(
      view
        .getViewElements()
  ...
  test('makeHorizontal should add default styles for some properties', () => {
    view.makeHorizontal();
    expect(
      view.getViewElements().sliderTrack.getSliderTrack().style.height
    ).toBe('5px');
    expect(view.getViewElements().sliderTrack.getSliderTrack().style.top).toBe(
      '0px'
    );
    expect(view.getViewElements().firstHandle.getSliderHandle().style.top).toBe(
      '-5px'
    );
    expect(
      view.getViewElements().secondHandle.getSliderHandle().style.top
    ).toBe('-5px');
  });
});
